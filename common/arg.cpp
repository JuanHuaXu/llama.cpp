...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            params.speculative.draft.mtp_train_dump = value;
        }
    ).set_spec().set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}).set_env("LLAMA_ARG_SPEC_MTP_DUMP"));
    add_opt(common_arg(
        {"--spec-mtp-dump-limit"}, "N",
        "maximum number of draft-MTP training records to write (default: 0 = unlimited)",
        [](common_params & params, const std::string & value) {
            params.speculative.draft.mtp_train_dump_limit = std::stoull(value);
        }
    ...
    add_opt(common_arg(
        ...
        string_format("apply MTP hidden LoRA before exporting h_nextn so it steers recursive state (default: %s)",
                      params.speculative.draft.mtp_lora_state ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.speculative.draft.mtp_lora_state = value;
        }
    ).set_spec().set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}).set_env("LLAMA_ARG_SPEC_MTP_LORA_STATE"));
    add_opt(common_arg(
